...
namespace FunnyBlox
{
    public class Follower : MonoBehaviour
    {
        ...
        [LabelText("Скорость следования")]
        [SuffixLabel("_smoothing", Overlay = true)]
        [SerializeField]
        [Range(0.001f, 1f)]
        private float _smoothing = 0.5f;

        [LabelText("Использовать смещение стартовой позиции")]
        [SuffixLabel("_useOffset", Overlay = true)]
        [SerializeField]
        private bool _useOffset = true;

        [FoldoutGroup("Заморооженные оси")]
        [LabelText("Ось X")]
        [SuffixLabel("_freezeX", Overlay = true)]
        [SerializeField]
        private bool _freezeX = false;

        [FoldoutGroup("Заморооженные оси")]
        [LabelText("Ось Y")]
        [SuffixLabel("_freezeY", Overlay = true)]
        [SerializeField]
        private bool _freezeY = false;

        [FoldoutGroup("Заморооженные оси")]
        [LabelText("Ось Z")]
        [SuffixLabel("_freezeZ", Overlay = true)]
        [SerializeField]
        ...
        {
            if (!_target) {this.enabled = false; return;}
